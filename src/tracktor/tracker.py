...
class Tracker:
    ...
    def step(self, blob, cls='vehicle'):
        ...
        if self.public_detections:
            ...
            if cls == 'vehicle':
                boxes = blob['dets_v'].squeeze(dim=0)
                scores = blob['score_v'].squeeze(dim=0)
            else:
                boxes = blob['dets_p'].squeeze(dim=0)
                scores = blob['score_p'].squeeze(dim=0)
            boxes = boxes.detach().cuda()
            scores = scores.detach().cuda()
        else:
